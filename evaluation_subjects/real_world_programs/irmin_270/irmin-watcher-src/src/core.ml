...
module Dispatch = struct
  ...
  let apply t ~dir ~file =
    ...
    if log_on then Printf.printf "apply dir %s len %d\n%!" dir (List.length fns);
...
module Watchdog = struct
  ...
  let start { t; d } ~dir listen =
    ...
    | Some _ -> 
        let b = Dispatch.stats d ~dir <> 0 in
        Printf.printf "assertion %b\n%!" b;
        assert (b); 
        Lwt.return_unit
    | None   ->
        (* Note: multiple threads can wait here *)
        if log_on then  Printf.printf "Starting background watch mechanism\n%!";
        listen (fun file ->
            if log_on then  Printf.printf "Applying to dispatch\n%!";
            Dispatch.apply d ~dir ~file) >>= fun u ->
        if log_on then  Printf.printf "Adding to Watchdog's Hashtbl\n%!";
        Hashtbl.add t dir u;
...
let create listen =
  ...
  let listen id dir fn =
    ...
    if log_on then  Printf.printf "starting watch for id %d\n%!" id;
    Watchdog.start watchdog ~dir (listen dir) >|= fun () ->
    if log_on then  Printf.printf "adding callback  to dispatch for id %d\n%!" id;
