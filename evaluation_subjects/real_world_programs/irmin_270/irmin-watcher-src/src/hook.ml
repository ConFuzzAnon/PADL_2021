...
let rec poll n ~callback ~wait_for_changes dir files (event:event) =
  (* Printf.printf "poll n\n%!"; *)
  ( match event with
  | `Unknown -> 
      if log_on then Printf.printf "poll  read_files\n%!";
      read_files dir
  | `File f  ->
      let prefix = dir / "" in
  ...
  let process () =
    if Digests.is_empty diff then (
      if log_on then Printf.printf "Digests empty\n%!";
      Lwt.return_unit)
    else (
      if log_on then Printf.printf "Digests diff found\n%!";
      Log.debug (fun f -> f "[%d] polling %s: diff:%a" n dir Digests.pp diff);
      let files = Digests.files diff in
      Lwt_list.iter_p callback files)
  in
  process () >>= fun () ->
  if log_on then  Printf.printf "waiting for changes\n%!";
...
let v ~wait_for_changes ~dir callback =
  let n = !id in incr id;
  if log_on then Printf.printf "v read_files\n%!";
